...
function ProductDetails() {
  ...
  const addToCart = async () => {
      ...
      try {
          const { data } = await axios.post(
              `${import.meta.env.VITE_API_URL}/cart`, { 'productId': productDetails.id },
              {
                  headers: {
                      Authorization: `Tariq__${token}`,
                  },
                      ...
                      </div>
                      {
                        token ?
                          <button
                            ...
                            disabled={isAddingCart ?? "disabled"}
                          >
                            {isAddingCart ? (
                              <div className="spinner-border" role="status"></div>
                            ) : (
                  ...
                  <div>
                    <div className="d-flex flex-column gap-3   ">
                      {productDetails.reviews
                        ...
                        : <div className="mt-5 d-flex justify-content-center"><h4 className="text-secondary">No reviews</h4></div>}
                    </div>
                  </div>
                </div>
              </>
